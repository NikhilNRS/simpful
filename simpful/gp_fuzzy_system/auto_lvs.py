...
class FuzzyLinguisticVariableProcessor:
    ...
    def __init__(
        ...
        Args:
            ...
            exclude_columns_csv (str, optional): Path to the CSV file that contains the column names to exclude.
        ...
        if exclude_columns_csv:
            self.exclude_columns = self._load_exclude_columns_from_csv(exclude_columns_csv)
        else:
    ...
    def _load_terms_dict(self):
        ...
        spec = importlib.util.spec_from_file_location(
            "terms_dict", self.terms_dict_path
        )
        ...
        if len(unique_points) < len(control_points):
            for i in range(1, len(unique_points)):
                if unique_points[i] - unique_points[i - 1] == 0:
                    ...
                    if self.verbose:
                        print(
                            f"Adjusted control point: {unique_points[i-1]} to {unique_points[i]}"
                        )
        ...
        for i in range(len(terms)):
            try:
                if self.mf_type == "triangular":
                    ...
                    if self.verbose:
                        print(
                            f"Defined triangular fuzzy set for term '{terms[i]}' with points: {control_points[i]}, {control_points[i+1]}, {control_points[i+2]}"
                        )
                elif self.mf_type == "gaussian":
                    mean = control_points[i + 1]
                    sigma = (control_points[i + 2] - control_points[i]) / 2
                    FS_list.append(
                        FuzzySet(function=Gaussian_MF(mean, sigma), term=terms[i])
                    )
                    if self.verbose:
                        print(
                            f"Defined gaussian fuzzy set for term '{terms[i]}' with mean: {mean}, sigma: {sigma}"
                        )
                elif self.mf_type == "sigmoid":
                    mean = control_points[i + 1]
                    slope = (control_points[i + 2] - control_points[i]) / 2
                    FS_list.append(
                        FuzzySet(function=Sigmoid_MF(mean, slope), term=terms[i])
                    )
                    if self.verbose:
                        print(
                            f"Defined sigmoid fuzzy set for term '{terms[i]}' with mean: {mean}, slope: {slope}"
                        )
            except IndexError:
                if self.verbose:
                    print(
                        f"Skipping term '{terms[i]}' due to insufficient control points."
                    )
                break
        return FS_list
    ...
    def create_linguistic_variable(self, column_data, column_name, terms):
        ...
        while num_terms >= 2:
            ...
            if len(control_points) < num_terms + 2:
                ...
                if self.verbose:
                    print(
                        f"Reducing number of terms to {num_terms} for column '{column_name}'"
                    )
                continue
            ...
            if len(FS_list) == num_terms:
                LV = LinguisticVariable(
                    FS_list=FS_list,
                    universe_of_discourse=[low, high],
                    concept=column_name,
                )
                if self.verbose:
                    print(
                        f"Created linguistic variable for column '{column_name}' with {num_terms} terms"
                    )
                return LV
            else:
                num_terms -= 1  # Reduce the number of terms and retry
        ...
        if self.verbose:
            print(
                f"Failed to create linguistic variable for column '{column_name}' with at least 2 terms"
            )
        return None
    ...
    def process_dataset(self, save_csv=True):
        ...
        for column in self.data.columns:
            ...
            if self.use_standard_terms:
                ...
                terms = self.terms_dict.get(
                    column, self.standard_terms
                )  # Default to standard terms if not in terms_dict

            LV = self.create_linguistic_variable(
                self.data[column].values, sanitized_column_name, terms
            )
        ...
        if save_csv:
            sanitized_columns = [
                self.sanitize_variable_name(col) for col in self.data.columns
            ]
    ...
    def process(self):
        ...
        for column in self.data.columns:
            ...
            if fuzzy_sets:
                lv = LinguisticVariable(
                    fuzzy_sets,
                    universe_of_discourse=[min(control_points), max(control_points)],
                    concept=column,
                )
