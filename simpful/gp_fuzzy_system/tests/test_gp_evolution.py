...
class TestGPEvolution(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.x_train = pd.read_csv(
            Path(__file__).resolve().parent / "gp_data_x_train.csv"
        )
        cls.y_train = pd.read_csv(
            Path(__file__).resolve().parent / "gp_data_y_train.csv"
        )
        ...
        missing_features = [
            ...
            if feature not in cls.x_train.columns
        ]
        if missing_features:
    ...
    def test_predict_with_fis(self):
        ...
        self.assertTrue(
            economic_health._rules, "economic_health should have rules initialized"
        )
        ...
        self.assertIsInstance(
            predictions, np.ndarray, "Should return a numpy array of predictions"
        )
        self.assertEqual(
            predictions.shape[0],
            len(self.x_train),
            "Should return one prediction per data row",
        )
...
class TestGeneticAlgorithm(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.assertEqual(
            len(self.fitness_scores),
            self.population_size,
            "Fitness scores should match population size",
        )

    def test_initialize_population(self):
        self.assertEqual(
            len(self.population), self.population_size, "Population size is incorrect"
        )
        for system in self.population:
            rules = system.get_rules()
            self.assertTrue(
                3 <= len(rules) <= 7,
                "Number of rules in system is out of expected range",
            )
            for rule in rules:
                num_clauses = rule.split(" THEN ")[0].count(" IS ")
                self.assertTrue(
                    num_clauses >= self.min_clauses_per_rule,
    ...
    def test_select_parents(self):
        ...
        self.assertEqual(
            len(parents), self.selection_size, "Selection size is incorrect"
        )
    ...
    def test_evolutionary_algorithm(self):
        ...
        self.assertEqual(
            len(new_population),
            self.population_size,
            "New population size is incorrect",
        )

    def test_genetic_algorithm_loop(self):
        self.assertGreater(
            self.max_generations, 0, "max_generations must be greater than 0"
        )
        print(f"max_generations: {self.max_generations}")

        best_system, best_fitness_per_generation, average_fitness_per_generation = (
            genetic_algorithm_loop(
                self.population_size,
        ...
        best_fitness_df = pd.DataFrame(
            best_fitness_per_generation, columns=["Generation", "Best Fitness"]
        )
        average_fitness_df = pd.DataFrame(
            average_fitness_per_generation, columns=["Generation", "Average Fitness"]
        )
        ...
        self.assertIsNotNone(
            best_system.evaluate_fitness(self.variable_store),
            "The best system should have a fitness score",
        )
