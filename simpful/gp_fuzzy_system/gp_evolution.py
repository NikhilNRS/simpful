...
from simpful.gp_fuzzy_system.gp_utilities import (
    ...
    find_best_models,
    replace_worst_models_with_best,
)
from simpful.gp_fuzzy_system.rule_generator import RuleGenerator
from simpful.gp_fuzzy_system.model_saver import (
    save_to_timestamped_dir,
    load_saved_individuals,
    load_populations_and_best_models,
)
import numpy as np
import logging
...
logging.basicConfig(
    filename="tests/evaluation_errors.log",
    level=logging.ERROR,
...
def initialize_population(
    ...
    available_features,
    min_rules=3,
    max_rules_per_system=7,
    min_clauses_per_rule=2,
    verbose=False,
    x_train=None,
    y_train=None,
    x_test=None,
    ...
    for system_index in range(population_size):
        ...
        system.available_features = (
            available_features  # Ensure available_features is always assigned
        )
        ...
        while not valid_rules_added:
            ...
            for rule in rules:
                try:
                    ...
                    if verbose:
                        logging.info(
                            f"Successfully added rule to system {system_index}: {rule}"
                        )
            ...
            if not valid_rules_added:
                logging.warning(
                    f"System {system_index} did not meet rule requirements. Retrying with a new set of rules."
                )
                system = EvolvableFuzzySystem()
                system.load_data(
                    x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test
...
def initialize_backup_population(
    population_size,
    variable_store,
    max_rules,
    available_features,
    ...
    for i in range(0, len(parents), 2):
        ...
        try:
            ...
            if child1 and child2:
                ...
                if verbose:
                    print(
                        f"Crossover did not produce valid offspring for parents {i} and {i + 1}"
                    )
        except Exception as e:
            if verbose:
                print(
                    f"Error during crossover of parent1 {parent1} and parent2 {parent2}: {e}"
                )
    if verbose:
...
def evaluate_population(
    ...
    for i in range(len(population)):
        ...
        logging.info(
            f"Evaluating system {i} (Generation {generation}/{max_generations}) with rules: {population[i].get_rules()}"
        )
        logging.info(
            f"Available features for system {i}: {population[i].available_features}"
        )
        ...
        while fitness_score is None and replacement_attempts < max_attempts:
            try:
                ...
                if verbose:
                    logging.info(
                        f"System {i} fitness evaluated successfully with score: {fitness_score}"
                    )
                ...
                if backup_population:
                    try:
                        ...
                        logging.info(
                            f"Replaced failed system {i} with a backup system."
                        )
                        logging.info(
                            f"Backup system rules for system {i}: {population[i].get_rules()}"
                        )

                    except Exception as backup_e:
                        # Log if the backup system also fails
                        logging.error(
                            f"Backup system also failed for system {i}: {backup_e}"
                        )
                    ...
                    logging.warning(
                        f"Backup system attempts for system {i}: {replacement_attempts}"
                    )

                    # Refill backup population if empty
                    refill_backup_population(
                        backup_population,
                        variable_store,
                        max_rules,
                        available_features,
        ...
        if fitness_score is None:
            fitness_score = float(
                "inf"
            )  # Assign a very high fitness score to indicate failure
            fitness_scores.append(fitness_score)
            logging.error(
                f"Failed to evaluate system {i} after {max_attempts} attempts. Assigned fitness score: inf"
            )

        # Log success or failure after all attempts
        if fitness_score != float("inf"):
            logging.info(
                f"System {i} successfully evaluated with fitness score: {fitness_score}"
            )
        else:
            logging.error(
                f"System {i} evaluation failed after maximum attempts, fitness score: inf"
            )

    return fitness_scores


def refill_backup_population(
    backup_population,
    variable_store,
    max_rules,
    available_features,
    ...
    new_backup_population = initialize_population(
        population_size * 3,
        variable_store,
        max_rules,
        available_features,
    ...
    if verbose:
        print(
            f"Refilled backup population with {len(new_backup_population)} new systems."
        )
...
def evolutionary_algorithm(
    ...
    if elitism_rate > 0:
        ...
        elites = sorted(zip(population, fitness_scores), key=lambda x: x[1])[
            :num_elites
        ]  # Sort ascending for minimizing
        new_population = [elite[0] for elite in elites] + offspring[
            : len(population) - num_elites
        ]
    else:
    ...
    if best_model not in new_population:
        ...
        worst_model_index = np.argmax(
            [
                individual.evaluate_fitness(variable_store)
...
def initialize_main_population(
    population_size,
    variable_store,
    max_rules,
    available_features,
    ...
    if seed_population_from and num_seed_individuals > 0:
        seeded_individuals = load_saved_individuals(
            seed_population_from, num_seed_individuals
        )
        remaining_population = initialize_population(
            population_size - len(seeded_individuals),
            variable_store,
            max_rules,
            available_features=available_features,
            x_train=x_train,
...
def load_and_initialize_population(
    ...
    if load_from:
        try:
            ...
            population = initialize_main_population(
                population_size,
                variable_store,
                max_rules,
                available_features,
...
def initialize_algorithm(
    ...
    backup_population = initialize_backup_population(
        population_size,
        variable_store,
        max_rules,
        available_features,
...
def genetic_algorithm_loop(
    ...
    for generation in range(max_generations):
        current_mutation_rate = adaptive_mutation_rate(
            mutation_rate, generation, max_generations
        )
        current_crossover_rate = adaptive_crossover_rate(
    ...
    best_system, final_fitness_scores = finalize_algorithm(
        progress_bar,
        population,
        variable_store,
        backup_population,
    ...
    return (
        best_system,
        list(zip(range(len(best_fitness_per_generation)), best_fitness_per_generation)),
        list(
            zip(
                range(len(average_fitness_per_generation)),
                average_fitness_per_generation,
            )
        ),
    )
